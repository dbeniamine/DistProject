...
and automate the simulation process. Its functioning is also explained at the
...
sending events, he should simply types ``start'' for the simulator to do a
...
The only events that is available (for now) is called ``broadcast'' and tells
...
start
\end{verbatim}
It can be used to measure the latency of a protocol for example.
...
perl script \textit{exp.pl} which allows the user to run every experiments
presented in this report and verify our results.

This script execute the program \textit{Broadcast} for different numbers of
nodes and displays the latency and the throughput of each runs. If the reader
wishes to use the scipt, he is advised to run it with no arguments to display
the ``help'' section.
...
it do perform the broadcast. Since the broadcast is propagated on a tree which
...
\bigskip
\noindent\textbf{No creation.} Delivery of messages $m$ by a process $p$ happen
...
In the second case, the message $m$ is deliverd by $p$ in response of a
...
received it before by its parent and so on up to the root of the tree, which
...
properly delivered in the sibling branches of the tree independently of whan
happen for $p$.
...
to its children it sends this message to all of his children without
interspering other messaging during the process. This ensures a total order
