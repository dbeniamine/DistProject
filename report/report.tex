...
protocol has a latency of $\log_2(N)$ where $N$ is the number of nodes.
...
broadcast protocol, i.e $\log_2(N)$. However, when a node that is not the
relay wished to broadcast a message, it first needs to unicast the message
to the relay, which take one round. The latency is then $\log_2(N) + 1$.

We give two different results for the latency:
\begin{itemize}
    \item \textbf{Worst case latency:} $\log_2(N) + 1$
    \item \textbf{Mean latency:} $\frac{1}{N} \log_2(N) + \frac{N-1}{N} (\log_2(N) + 1)
	                          = \log_2(N) + \frac{1}{N}$
\end{itemize}
